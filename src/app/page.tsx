...
import React, { useState, useRef, useEffect, type ChangeEvent, FormEvent } from 'react'; import Head from 'next/head';
...
export default function Home() {
    ...
    const handleLogin = (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        if (username === 'admin' && password === 'P@ssw0rd') { // Hardcoded credentials
    ...
    return (
        <main className="flex flex-col items-center justify-center min-h-screen w-full bg-background p-4 sm:p-6 md:p-8 overflow-auto">
            {!isAuthenticated ? (
                <Card className="w-full max-w-sm shadow-xl rounded-2xl p-6">
                    <CardHeader className="text-center p-0 mb-6">
                        <CardTitle className="font-headline text-3xl font-bold tracking-tight text-foreground">Login</CardTitle>
                        <CardDescription className="text-muted-foreground pt-2">
                            Enter your credentials to access InstaSquare.
                        </CardDescription>
                    </CardHeader>
                    <CardContent className="p-0">
                        <form onSubmit={handleLogin} className="grid gap-4">
                            ...
                            {isLoginLocked && (
                                <p className="text-sm text-destructive text-center">Too many failed attempts. Please try again later.</p>
                            )}
                            <Button type="submit" className="w-full" disabled={isLoginLocked}>
                                Login
                            </Button>
                        </form>
                    </CardContent>
